...
public class ReviewController {
    ...
    public ResponseEntity<ReviewResponseDto> addReview(@Valid @RequestBody ReviewRequestDto reviewRequestDto) {
       ...
       .body(reviewService.saveReview(reviewRequestDto));
    }

    @GetMapping("/{imdbId}")
    public ResponseEntity<List<ReviewResponseDto>> getMethodName(@PathVariable String imdbId) {
        return ResponseEntity.ok(reviewService.getReviewsByMovie(imdbId));
    }
